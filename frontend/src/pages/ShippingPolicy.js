...
const ShippingPolicy = () => {
  ...
  useEffect(() => {
    api.get('/api/pages/shipping-policy')
    ...
    return (
      <div className="container" style={{ padding: '60px 24px', maxWidth: 800, margin: '0 auto' }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center" style={{ padding: '60px 20px' }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display', serif", color: '#2E5A44', marginBottom: 8 }}>Shipping Policy</h2>
          <p style={{ color: '#9ca3af', fontSize: 15 }}>Shipping policy information is not available yet. Please check back later or contact support.</p>
        </motion.div>
      </div>
    );
  }
  ...
  return (
    <div className="container" style={{ padding: '60px 24px', maxWidth: 800, margin: '0 auto' }}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center" style={{ marginBottom: 48 }}>
        ...
        <h1 style={{ fontFamily: "'Playfair Display', serif", color: '#2E5A44', marginBottom: 8 }}>{page?.title || 'Shipping Policy'}</h1>
      ...
      <div style={{ marginTop: 40, padding: 24, background: '#f0f7f1', borderRadius: 12, textAlign: 'center' }}>
        <p style={{ color: '#2E5A44', fontSize: 14, lineHeight: 1.6 }}>
          Have questions about shipping? Please <a href="/contact" style={{ color: '#2E5A44', fontWeight: 600, textDecoration: 'underline' }}>contact us</a>.
        </p>
      </div>
    </div>
  );
